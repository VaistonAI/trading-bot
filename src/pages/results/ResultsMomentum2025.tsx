...
export const ResultsMomentum2025 = () => {
    const [results, setResults] = useState<SimulationResult | null>(null);
    const [isLoading, setIsLoading] = useState(true);
    const [isRunning, setIsRunning] = useState(false);
    const [isSaving, setIsSaving] = useState(false);
    const [showSaveButton, setShowSaveButton] = useState(false);
    const [selectedYear, setSelectedYear] = useState(2023); // Tab de aÃ±o seleccionado
    ...
    const loadResults = async () => {
        try {
            setIsLoading(true);
            const docRef = doc(db, 'simulations-momentum', '2025');
                ...
                <div className="mb-6 flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-bold text-text-primary mb-2">
                            R Momentum (2023-2025)
                        </h1>
                        <p className="text-text-secondary">
                            Simulación histórica de la estrategia Momentum en 2025 (YTD: Ene 1 - Dic 14)
                        </p>
                    </div>
                ...
                {results ? (
                                    ...
                                    <tbody>
                                        {results.monthlyBreakdown
                                            .filter(month => {
                                                ...
                                                // Fallback: datos antiguos (12 meses sin aÃ±o)
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-border p-12 text-center">
                        ...
                        <p className="text-text-secondary mb-6">
                            Ejecuta la simulación para ver el rendimiento de la estrategia Momentum en 2025
                        </p>
                    </div>
                )}
            </div>
