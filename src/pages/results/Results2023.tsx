...
export const Results2023: React.FC = () => {
    ...
    const loadResults = async () => {
        try {
            setIsLoading(true);
            const docRef = doc(db, 'simulations', '2023');
    ...
    const runSimulation = async () => {
        try {
            ...
            if (response.ok) {
                ...
                setResults(data);
                setShowSaveButton(true); // Mostrar botón guardar
            } else {
    ...
    const saveToFirebase = async () => {
        ...
        try {
            ...
            const docRef = doc(db, 'simulations', results.year.toString());
            await setDoc(docRef, {
                ...results,
            ...
            await loadResults(); // Recargar desde Firebase
        } catch (error) {
                ...
                {results ? (
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-border p-12 text-center">
                        ...
                        <p className="text-text-secondary mb-6">
                            Ejecuta la simulación para ver el rendimiento histórico de 2023
                        </p>
                    </div>
                )}
            </div>
