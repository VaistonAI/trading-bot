...
import { FaChartLine, FaRocket, FaDollarSign } from 'react-icons/fa';
...
export const MomentumStrategy: React.FC = () => {
    ...
    const STRATEGY_ID = 'momentum-trading'; // ID fijo para esta estrategia
    ...
    const loadStrategy = async () => {
        setIsLoading(true);
        try {
            // Intentar obtener estrategia existente
            let existingStrategy = await strategyService.getById(STRATEGY_ID);

            if (!existingStrategy) {
                // Crear estrategia si no existe
                const newStrategyData: Omit<Strategy, 'id'> = {
                    name: 'Momentum Trading',
                    type: StrategyType.MOMENTUM,
                    description: 'Estrategia basada en el impulso del mercado. Identifica acciones con tendencias alcistas fuertes usando RSI, MACD y Moving Averages.',
                    isActive: true,
                    telegramNotificationsEnabled: false,
                    liveAlertsEnabled: false,
                    virtualCapital: 10000, // $10,000 MXN inicial
        ...
        return (
            <MainLayout>
                <div className="text-center py-12">
                    <p className="text-text-secondary">Error al cargar la estrategia</p>
                </div>
            </MainLayout>
        );
    }
    ...
    return (
        <MainLayout>
            <div className="space-y-6">
                {/* Header */}
                <div>
                    <h1 className="text-3xl font-bold text-text-primary">Momentum Trading</h1>
                ...
                <div className="bg-white rounded-xl shadow-md border border-border overflow-hidden">
                    ...
                    <div className="p-6">
                        {activeTab === 'overview' && (
                            <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                    <div>
                                        <h3 className="text-lg font-semibold text-text-primary mb-4">Indicadores Clave</h3>
                                        <div className="space-y-3">
                                            <div className="p-4 bg-bg-primary rounded-lg">
                                                <p className="text-sm text-text-secondary">RSI (14)</p>
                                                <p className="text-xl font-bold text-text-primary">Señal de compra cuando &gt; 60</p>
                                            </div>
                                            <div className="p-4 bg-bg-primary rounded-lg">
                                                <p className="text-sm text-text-secondary">MACD</p>
                                                <p className="text-xl font-bold text-text-primary">Cruce alcista de líneas</p>
                                            </div>
                                            <div className="p-4 bg-bg-primary rounded-lg">
                                                <p className="text-sm text-text-secondary">Moving Averages</p>
                                                <p className="text-xl font-bold text-text-primary">Precio sobre MA20, MA50, MA200</p>
                                            </div>
                                        </div>
                                    </div>
                                    <div>
                                        <h3 className="text-lg font-semibold text-text-primary mb-4">Reglas de la Estrategia</h3>
                                        <div className="space-y-3">
                                            <div className="p-4 bg-success/10 rounded-lg border border-success/20">
                                                <p className="text-sm font-semibold text-success mb-2">✓ Compra</p>
                                                <ul className="text-xs text-text-secondary space-y-1">
                                                    <li>• RSI &gt; 60</li>
                                                    <li>• Precio cruza MA20 al alza</li>
                                                    <li>• Volumen superior al promedio</li>
                                                    <li>• MACD positivo</li>
                                                </ul>
                                            </div>
                                            <div className="p-4 bg-danger/10 rounded-lg border border-danger/20">
                                                <p className="text-sm font-semibold text-danger mb-2">✗ Venta</p>
                                                <ul className="text-xs text-text-secondary space-y-1">
                                                    <li>• RSI &lt; 40</li>
                                                    <li>• Precio cruza MA20 a la baja</li>
                                                    <li>• MACD negativo</li>
                                                    <li>• Stop loss: -5%</li>
                                                </ul>
                                            </div>
                        ...
                        {activeTab === 'reports' && (
                            <ReportTab performance={strategy.performance} />
                        )}
                    </div>
                </div>
            </div>
